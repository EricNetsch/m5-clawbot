 ...
 * @note Sends on the device's CURRENT channel (channel 7, set at init), exactly
 *       like the factory OOTB firmware. Channel 7 is the agreed-upon fixed
 *       ESP-NOW channel between this remote and the stackchan robot.
 */
void espnow_send_data(uint8_t *pkt, size_t len)
{
    espnow_frame_head_t frame_head = ESPNOW_FRAME_CONFIG_DEFAULT();
    esp_err_t e = espnow_send(ESPNOW_DATA_TYPE_DATA, ESPNOW_ADDR_BROADCAST, pkt, len,
                              &frame_head, portMAX_DELAY);
    if (e != ESP_OK) {
        ESP_LOGW("espnow_send", "espnow_send -> %s", esp_err_to_name(e));
    }
}