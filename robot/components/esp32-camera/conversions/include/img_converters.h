...
/* NOTE (stackchan): JPEG *decode* (jpeg_decoder.h / esp_jpeg) is not built on
 * this device -- only encode (jpge.cpp/to_jpg.cpp) is used. The decode-only
 * declarations below are #if 0'd out so the header compiles without esp_jpeg. */
...
// Macros for backwards compatibility (JPEG decode; not built on this device)
#if 0
