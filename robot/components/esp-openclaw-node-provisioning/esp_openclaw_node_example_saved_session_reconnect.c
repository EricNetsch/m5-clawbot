...
static void esp_openclaw_node_example_saved_session_reconnect_task(void *arg)
{
    ...
    for (;;) {
        /* Periodic tick (not portMAX_DELAY): the transport can fail for reasons
         * that never notify this task (e.g. CANNOT_RESOLVE_HOSTNAME while the
         * WireGuard tunnel to the gateway is still coming up). Without a timed
         * retry the task parks forever and the node stays disconnected even
         * after the tunnel establishes. A ~5s tick makes reconnect self-healing;
         * request_connect returns INVALID_STATE (handled silently) when already
         * connected/connecting, so idle ticks are cheap no-ops. */
