...
void esp_openclaw_node_free_config_strings(esp_openclaw_node_config_t *config)
{
    ...
    free((char *)config->identity_nvs_namespace);
    ...
    esp_openclaw_node_config_t *dst)
{
    ...
    ESP_RETURN_ON_ERROR(
        copy_string_field(
            (char **)&dst->identity_nvs_namespace,
            src->identity_nvs_namespace,
            false),
        ESP_OPENCLAW_NODE_TAG,
        "identity_nvs_namespace");
    ESP_RETURN_ON_ERROR(
        copy_string_field(
    ...
    esp_openclaw_node_handle_t *out_node)
{
    ...
    err = esp_openclaw_node_identity_load_or_create_ns(
        &node->identity,
        node->config.identity_nvs_namespace);
    if (err != ESP_OK) {
        esp_openclaw_node_free_config_strings(&node->config);
        vQueueDelete(node->work_queue);
        vSemaphoreDelete(node->destroy_done);
