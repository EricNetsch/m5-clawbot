...
typedef struct {
    ...
    const char *identity_nvs_namespace; /**< Optional NVS namespace for this handle's device identity/seed. NULL uses the default. A distinct namespace yields a distinct device_id, enabling a second (e.g. node-role) handle on the same chip to pair as its own device. */
