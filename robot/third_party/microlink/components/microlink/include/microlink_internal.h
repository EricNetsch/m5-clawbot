...
/* Cadence for re-firing a WG handshake init to a peer that has no active
 * session yet. Driven off DISCO PONGs (~every few seconds). */
#define ML_WG_HANDSHAKE_RETRY_MS        5000
...
typedef struct {
    ...
    /* Periodic handshake retry: last time (ms) we fired a handshake init to this
     * peer while it had no active WG session. Lets us keep retrying every
     * ML_WG_HANDSHAKE_RETRY_MS instead of giving up after one attempt, which is
     * required when the peer initially rejects us (e.g. replay/timestamp) but
     * will accept a later init. 0 = never attempted. */
